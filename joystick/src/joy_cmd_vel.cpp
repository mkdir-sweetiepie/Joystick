...
void Joy_cmd_vel::operate()
{
  ...
  if (target_angular1 > ANGULAR1_MAX)
  {
    target_angular1 = ANGULAR1_MAX;
  }
  else if (target_angular1 < -ANGULAR1_MAX)
  {
    target_angular1 = -ANGULAR1_MAX;
  }

  // 현재 각속도2의 값이 1.0 ~ -1.0 사이에 있도록 함.
  if (target_angular2 > ANGULAR2_MAX)
  {
    target_angular2 = ANGULAR2_MAX;
  }
  else if (target_angular2 < -ANGULAR2_MAX)
  {
    target_angular2 = -ANGULAR2_MAX;
  }

  twist.linear.x = target_angular1;
  twist.angular.z = target_angular2;
